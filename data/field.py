...
class ImageDetectionsField(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=100,
        ...
        self.word_dic = ["background"]
        print("load object label dic")
        for word in open('./data/objects_vocab.txt'):
            self.word_dic.append(word[:-1])
        self.word_dic.append('<pad>')
        if load_in_tmp:
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        try:
            ...
            bbox = f['%d_boxes' % image_id][()]
            cls = f['%d_cls_prob' % image_id][()].argmax(axis=1)
            if self.sort_by_prob:
                precomp_data = precomp_data[np.argsort(np.max(f['%d_cls_prob' % image_id][()], -1))[::-1]]
                bbox = bbox[np.argsort(np.max(f['%d_cls_prob' % image_id][()], -1))[::-1]]
                cls = cls[np.argsort(np.max(f['%d_cls_prob' % image_id][()], -1))[::-1]]

        except KeyError:
            warnings.warn('Could not find detections for %d' % image_id)
            precomp_data = np.random.rand(10,2048)
            cls = np.random.rand(10)
            bbox = np.random.rand(10,4)

        delta = self.max_detections - precomp_data.shape[0]
        if delta > 0:
            precomp_data = np.concatenate([precomp_data, np.zeros((delta, precomp_data.shape[1]))], axis=0)
            bbox = np.concatenate([bbox, np.zeros((delta, bbox.shape[1]))], axis=0)
            cls = np.concatenate([cls, np.full((delta),1601,dtype=np.int64)], axis=0)
        elif delta < 0:
            precomp_data = precomp_data[:self.max_detections]
            bbox = bbox[:self.max_detections]
            cls = cls[:self.max_detections]

        dic = self.build_word(cls)
        cls = np.array(self.word_to_onehot(dic))
        return precomp_data.astype(np.float32),bbox.astype(np.float32),cls.astype(np.int64)


    def build_word(self,cls):
        dic = []
        for i in cls.tolist():
            dic.append(self.word_dic[int(i)])
        return dic



    def word_to_onehot(self,dic):
        vocab = pickle.load(open('./vocab_MyImageCaption.pkl', 'rb'))
        cls = []
        for word in dic:
            cls.append(vocab.stoi[word])
