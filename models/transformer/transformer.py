...
class Transformer(CaptioningModel):
    ...
    def forward(self,images,bbox,labels,seq,*args):

        enc_output, mask_enc = self.encoder(images,bbox,labels)
    ...
    def step(self, t, prev_output,visual,bbox,labels,seq, mode='teacher_forcing', **kwargs):
        it = None
        if mode == 'teacher_forcing':
            raise NotImplementedError
        elif mode == 'feedback':
            if t == 0:
                self.enc_output, self.mask_enc = self.encoder(visual,bbox,labels)
