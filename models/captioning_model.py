...
class CaptioningModel(Module):
    ...
    def sample_rl(self, visual: utils.TensorOrSequence,bbox: utils.TensorOrSequence,labels: utils.TensorOrSequence, max_len: int, **kwargs) -> utils.Tuple[torch.Tensor, torch.Tensor]:
        b_s = utils.get_batch_size(visual)
        outputs = []
        log_probs = []
        ...
        with self.statefulness(b_s):
            out = None
            for t in range(max_len):
                out = self.step(t, out, visual,bbox,labels, None, mode='feedback', **kwargs)
    ...
    def beam_search(self, visual: utils.TensorOrSequence,bbox: utils.TensorOrSequence,labels: utils.TensorOrSequence, max_len: int, eos_idx: int, beam_size: int, out_size=1,
                    return_probs=False, **kwargs):
        bs = BeamSearch(self, max_len, eos_idx, beam_size)
        return bs.apply(visual,bbox,labels, out_size, return_probs, **kwargs)
