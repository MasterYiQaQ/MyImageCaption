...
class BeamSearch(object):
    ...
    def _expand_state(self, selected_beam, cur_beam_size):
        def fn(s):
            shape = [np.int64(sh) for sh in s.shape]
            beam = selected_beam
            for _ in shape[1:]:
                beam = beam.unsqueeze(-1)
            s = torch.gather(s.view(*([np.int64(self.b_s), np.int64(cur_beam_size)] + shape[1:])), 1,
                             beam.expand(*([np.int64(self.b_s), np.int64(self.beam_size)] + shape[1:])))
            s = s.view(*([-1, ] + shape[1:]))
            return s
        return fn
    ...
    def apply(self, visual: utils.TensorOrSequence,bbox: utils.TensorOrSequence,labels: utils.TensorOrSequence, out_size=1, return_probs=False, **kwargs):
        ...
        with self.model.statefulness(self.b_s):
            for t in range(self.max_len):
                visual,bbox,labels,outputs = self.iter(t, visual,bbox,labels,outputs, return_probs, **kwargs)
    ...
    def iter(self, t: int, visual: utils.TensorOrSequence,bbox: utils.TensorOrSequence,labels: utils.TensorOrSequence, outputs, return_probs, **kwargs):
        cur_beam_size = 1 if t == 0 else self.beam_size

        word_logprob = self.model.step(t, self.selected_words,visual,bbox,labels,None, mode='feedback', **kwargs)
        ...
        labels = self._expand_visual(labels, cur_beam_size, selected_beam)
        bbox = self._expand_visual(bbox, cur_beam_size, selected_beam)
        ...
        return visual,bbox,labels,outputs
