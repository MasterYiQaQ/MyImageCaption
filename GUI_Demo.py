...
def predict_caption(file_url, beam_size=1, show_objs=False):
    ...
    if file_url.startswith('http'):
        ...
        file_url = './temp.jpg'
        load_online = True
    objs, bbox, probs, labels, im = proposals.process_image(file_url, detectionModel, text_field, args, 50, device,
                                                            show_objs)
    ...
    file_list_column = [
        ...
        [
            ...
            gui.Listbox(
                ...
                values=[], enable_events=True, size=(40, 20), key="-FILE LIST-", select_mode='single',
        ...
        if event == "Generate Caption":  # for generating the caption of the image
            try:
                if filename:
                    ...
                    window["-CAPTION-"].update(caps, background_color='#856ff8', font=("Times New Roman", 12),
                                               text_color='Yellow')

                else:

                    window["-CAPTION-"].update("No image is selected.", background_color='#856ff8',
                                               font=("Times New Roman", 11), text_color='Red')
